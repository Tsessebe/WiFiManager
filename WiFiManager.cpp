...
void WiFiManager::showCustomPage(String body)
{
#ifdef WM_DEBUG_LEVEL
  ...
  HTTPSend(page);
  if (_preloadwifiscan)
    WiFi_scanNetworks(_scancachetime, true);
}
...
String WiFiManager::getMenuOut()
{
  ...
  {
    if (strcmp_P("param", _menutokens[menuId]) == 0 && _paramsCount == 0)
    {
  ...
  else if (id == F("uptime"))
  {
    ...
    if (min >= 60)
    {
      ...
      if (hrs >= 24)
      {
        days = hrs / 24;
        hrs = hrs % 24;
        p = FPSTR(HTTP_INFO_uptime_D);
        p.replace(FPSTR(T_1), (String)(days));
        p.replace(FPSTR(T_2), (String)(hrs));
        p.replace(FPSTR(T_3), (String)(min));
        p.replace(FPSTR(T_4), (String)(sec));
      }
      else
      {
        p = FPSTR(HTTP_INFO_uptime_H);
        p.replace(FPSTR(T_1), (String)(hrs));
        p.replace(FPSTR(T_2), (String)(min));
        p.replace(FPSTR(T_3), (String)(sec));
      }
    }
    else
    {
      p = FPSTR(HTTP_INFO_uptime);
      p.replace(FPSTR(T_1), (String)(min));
      p.replace(FPSTR(T_2), (String)(sec));
    }
  }
  else if (id == F("chipid"))
  {
